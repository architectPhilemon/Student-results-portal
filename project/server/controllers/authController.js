...
const login = async (req, res) => {
  try {
    ...
    // Emit login event for real-time notifications
    const io = req.app.get('io');
    if (io) {
      io.to(`user_${user._id}`).emit('user_login', {
        userId: user._id,
        timestamp: new Date()
      });
    }
...
const logout = async (req, res) => {
  try {
    ...
    // Emit logout event for real-time notifications
    const io = req.app.get('io');
    if (io) {
      io.to(`user_${req.user.id}`).emit('user_logout', {
        userId: req.user.id,
        timestamp: new Date()
      });
    }
